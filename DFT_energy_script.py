...
for file in files:
  ...
  if name.split(".")[1]=='sdf':
      ...
      with open(script_dir+order+".gjf", "a+") as f:
          ...
          current_atom=1
          bond_line=[0]
          while current_atom <= int(atom_num):
              bond_line.append(" "+str(current_atom))
              for line in bond_info:
                  if line[2]==str(current_atom):
                      bond_line[current_atom]=bond_line[current_atom]+" "+line[5]+" "+line[8]+".0"
                  else:
                      pass
              current_atom+=1
          i=1
          while i < len(bond_line):
              f.write(bond_line[i])
              f.write("\n")
              i+=1
          f.close()
